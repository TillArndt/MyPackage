...
# version: V07-00-03      TopQuarkAnalysis/Configuration 
#
...
try:
    ...
    print "<patRefSel_muJets_cfg>: cms_var not in __builtin__!"
print "<patRefSel_muJets_cfg>: Running On MC:", runOnMC
...
inputFiles = ['file:/disk1/tholen/eventFiles/syncExercise52.root']
...
# GlobalTags
globalTagData = 'FT_53_V6C_AN3::All'
globalTagMC   = 'START52_V11::All'
...
process.source.skipBadFiles = False
...
# Custom modifications from SyncExercise
########################################

process.MessageLogger.suppressInfo = ["TimeReport"]

# step 0
process.step0a.triggerConditions = ["HLT_IsoMu20_eta2p1_TriCentralPFJet30_v2"]

# step 1
process.p.remove(process.CSCTightHaloFilter)

# step 3
process.selectedPatElectronsPF.cut = 'pt > 20. && abs(eta) < 2.5 && electronID("mvaTrigV0") > 0. && (chargedHadronIso+max(0.,neutralHadronIso)+photonIso-0.5*puChargedHadronIso)/et < 0.15'

# step 4
#process.patJets.jetSource = cms.InputTag("pfJets")
#process.patJetPartonAssociation.jets = cms.InputTag("pfJets") 
#process.patJetPartonMatch.src = cms.InputTag("pfJets") 
#process.patJetGenJetMatch.src = cms.InputTag("pfJets") 
#process.jetTracksAssociatorAtVertex.jets = cms.InputTag("pfJets") 
#process.patJetCorrFactors.src = cms.InputTag("pfJets") 
#process.softMuonTagInfosAOD.jets = cms.InputTag("pfJets")
